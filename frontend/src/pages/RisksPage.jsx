...
export default function RisksPage() {
  ...
  const { data: issues, loading: loadingIssues } = useIssues({ project_id: projectId })
    ...
    <div className="min-h-screen bg-canvas">
      <header className="border-b border-border bg-white">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center gap-4">
          ...
          <h1 className="font-display text-xl font-bold">Proyecto</h1>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center gap-1 border border-border rounded-lg p-1 bg-white w-fit mb-8">
          <button
            onClick={() => { setTab('risks'); setShowForm(false) }}
          ...
          <button
            onClick={() => { setTab('issues'); setShowForm(false) }}
            className={`flex items-center gap-2 px-4 py-2 rounded text-sm font-medium transition-colors ${
        ...
        {tab === 'issues' && (
          <>
            <div className="mb-8">
              <h2 className="font-display text-3xl font-bold">Issues del proyecto</h2>
            ...
            {loadingIssues ? (
              ...
              <div className="text-center py-16 text-muted">
                ...
                <p className="text-sm">No hay issues en este proyecto. Se crean al derivar un riesgo.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
